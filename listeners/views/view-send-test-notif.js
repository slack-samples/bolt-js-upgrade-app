...
const viewSendTestNotif = async ({ ack, client, body, respond }) => {    
  try {
    ...
    // If the user selected themselves as the test message destination, send them a message via the bot DM
    // instead of with response_url which goes to their private DM with themselves.

    // If you select a DM with any other user, the message is actually posted to your DM with that user
    // rather than the Bot's DM with that user. This is just how the `response_url_enabled` arg works
    // for the conversation select in input blocks in modals.
